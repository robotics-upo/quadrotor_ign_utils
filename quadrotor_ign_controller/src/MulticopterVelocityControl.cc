...
#include "quadrotor_ign_controller/MulticopterVelocityControl.hh"
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  if (sdfClone->HasElement("thrustWithPayloadMass"))
  {
    this->thrustWithPayloadMass = sdfClone->Get<bool>("thrustWithPayloadMass");
  }
    ...
    const EntityComponentManager &_ecm, Entity _entity)
{
  ...
  for (const Entity &model :
       _ecm.ChildrenByComponents(_entity, components::Model()))
  {
    vehicleInertial += this->VehicleInertial(_ecm, model);
  }
  return vehicleInertial;
}
